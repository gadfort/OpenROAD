...
//
// select_id < 0 means "re-inspect whatever is currently inspected"
...
static gui::Selected resolveBySelectId(const WebSocketRequest& req,
                                       SessionState& state)
{
  const int select_id = extract_int(req.json, "select_id");
  if (select_id < 0) {
    std::lock_guard<std::mutex> lock(state.selection_mutex);
    return state.current_inspected;
  }
  std::lock_guard<std::mutex> lock(state.selectables_mutex);
...
static gui::Selected resolveByOdb(const WebSocketRequest& req,
                                  odb::dbBlock* block)
{
  const std::string odb_type = extract_string(req.json, "odb_type");
  if (odb_type.empty() || block == nullptr) {
    return {};
  }
  auto* registry = gui::DescriptorRegistry::instance();
  const uint32_t id
      = static_cast<uint32_t>(extract_int_or(req.json, "odb_id", -1));
  if (odb_type == "bterm") {
...
// resolution.
WebSocketResponse SelectHandler::buildInspectResponse(
...
WebSocketResponse SelectHandler::handleInspect(const WebSocketRequest& req,
                                               SessionState& state)
{
  return buildInspectResponse(req, resolveBySelectId(req, state), state);
}

WebSocketResponse SelectHandler::handleInspectByOdb(const WebSocketRequest& req,
                                                    SessionState& state)
{
  return buildInspectResponse(req, resolveByOdb(req, gen_->getBlock()), state);
}
